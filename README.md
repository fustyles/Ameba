...
### Gemini 同步時鐘
開機時，系統呼叫 `googleSearchTime()`，指示 Gemini 對設定時區的當前本地時間執行即時網路搜尋。Gemini 回傳包含年、月、日、時、分、秒欄位的結構化 JSON 物件。韌體將其直接解析並透過 `rtc.SetEpoch()` 寫入硬體 RTC。最終實現了**完全透過自然語言 AI 工具完成的網路同步硬體時鐘**——無需任何 NTP 函式庫。

### 時區感知排程
時區字串在執行期被附加至 `devicesDefinition`，成為 Gemini 可見的系統上下文的一部分。排程任務評估因此能夠存取正確的本地時間上下文，韌體本身不包含任何硬編碼的時區邏輯。更改時區只需編輯 `env.md` 即可。

### 排程任務的懶加載 RTC 初始化
`task_time_scheduling` 背景任務在每次評估週期前檢查 `rtcYear == 0`。若 RTC 在開機時未能成功初始化（例如因短暫的網路故障），它會在嘗試任何排程邏輯之前透明地重試同步。這種優雅降級防止了因時鐘未初始化而導致排程任務在錯誤時間觸發的問題。
