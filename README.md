...
7. [透過 Gemini 搜尋同步 RTC 時間](#7-透過-gemini-搜尋同步-rtc-時間)
...
## 7. 透過 Gemini 搜尋同步 RTC 時間

在沒有網路時間協定（NTP）函式庫的嵌入式裝置上實現時間感知，是一個非平凡的問題。fuClaw 以優雅的方式解決了這個挑戰。

### Gemini 同步時鐘
開機時，系統呼叫 `googleSearchTime()`，以純 JSON schema 為格式要求，直接透過 `geminiSearchRequest()` 向 Gemini 發送 prompt，要求回傳指定時區的當前本地時間。Gemini 執行即時網路搜尋後，回傳包含年、月、日、時、分、秒六個欄位的結構化 JSON 物件。韌體解析後先呼叫 `rtc.SetEpoch()` 計算對應的 epoch 時間戳，再以 `rtc.Write(initTime)` 寫入硬體 RTC。整個流程**完全繞過 NTP 函式庫**，以 AI 搜尋能力替代傳統時間同步協定。

### 時區感知排程
時區字串在 `setup()` 初始化階段以字串拼接的方式附加至 `devicesDefinition`，成為系統提示詞的一部分，使 Gemini 在排程評估時能存取正確的本地時間上下文，韌體本身不包含任何硬編碼的時區邏輯。更改時區只需編輯 `env.md` 即可。

### 排程任務的防護性 RTC 初始化
`task_time_scheduling` 背景任務在每次評估週期前先檢查 `rtcYear == 0`。若 RTC 在開機時未能成功初始化（例如因短暫的網路故障），任務會先呼叫 `rtcInitialTime()` 嘗試重新同步；若同步仍然失敗（`rtcYear` 依然為 0），則直接執行 `continue` **跳過本次排程週期**，而非以未初始化的時鐘進行錯誤的時間比對。這種「寧可不執行、也不錯誤執行」的保守策略，防止了排程任務在時鐘狀態不可信時誤觸發。
