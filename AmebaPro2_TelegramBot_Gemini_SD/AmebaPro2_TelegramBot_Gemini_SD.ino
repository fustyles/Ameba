...
Telegram User
      ↓
AMB82-mini (FreeRTOS background polling task)
      ↓
Gemini API (generateContent + optional Google Search tool)
      ↓
JSON tool_call parsing (ArduinoJson)
      ↓
...
not a native function calling framework.
...
- Prompt-based tool calling system (JSON tool_call output)
- Optional Google Search grounding via Gemini tool
- Camera capture and Telegram image upload (sendPhoto)
- Gemini Vision multimodal analysis (image + text input)
- Digital output control (GPIO ON/OFF)
- PWM output control (0–255 brightness)
- System memory monitoring (heap + message buffer size)
- Conversation memory stored in RAM (historical_messages + SD backup)
- FreeRTOS background task for Telegram polling
...
tool_call instructions when an action is required.

IMPORTANT:
- Tool routing is not enforced by API
- It depends entirely on model prompt compliance
- Arduino-side JSON validation is required for safety
...
4) Vision Analysis (camera + Gemini multimodal)
{
  "type": "tool_call",
  "method": "/vision",
  "params": {
...
------------------------------------------------------------
Execution Flow (Actual Firmware Behavior)
------------------------------------------------------------

1. Telegram message received via getUpdates (polling task)
2. Message is forwarded to Gemini API OR parsed as direct command
3. Gemini returns:
     - tool_call JSON (preferred)
     - or /chat response wrapper
4. AMB82-mini parses JSON using ArduinoJson library
5. useTools() dispatches execution:
     ...
     - Search via Gemini grounding tool
     - Memory handling (RAM + SD persistence)
     - Telegram response (HTTPS sendMessage/sendPhoto)
6. Result is sent back to Telegram Bot API
...
- Conversation history stored in RAM (historical_messages)
- Optional persistence via SD card (memory.md)
- System prompt dynamically injected at runtime
- All Gemini communication uses HTTPS POST
- Telegram uses long polling (no webhook)
- Tool execution depends on JSON parsing success
- Camera images are Base64 encoded for Gemini Vision API
- System is event-driven via FreeRTOS polling task

------------------------------------------------------------
Limitations (Important for AMB82-mini)
------------------------------------------------------------

- RAM usage increases with long conversations (no strict GC)
- Vision pipeline is memory intensive (Base64 + JSON)
- No true persistent vector memory (only text dump storage)
- Telegram polling is continuous (not webhook-based)
- JSON parsing depends on strict model output format
- TLS connections are relatively heavy on heap usage
...

{
  ...
  "method": "/on",
  "params": {
    "pin": "<LED pin number. If the user explicitly specifies a pin number, use that number. If the user does not specify which LED pin to control, first ask the user in normal conversational reply and wait for the user's answer before filling this value. If multiple pins are mentioned, choose only the first one in order of appearance.>",
	"pinmode": "digitalwrite",
    "value": 1
  }
}
